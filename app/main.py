    ...
    try:
        while True:
            ...
            if message_type == 'chat_message':
                ...
                elif widget_type == "ai_chat_widget":
                    userInput = widget_params.get('prompt')
                    if userInput:
                        async def deep_handler(current_websocket,current_prompt):
                            ai_response = await widgets.simple_ai_response(current_prompt)
                            ai_message = {"type":"widget_data","widget_type":"ai_chat_widget",
                                          "prompt":current_prompt,"response":ai_response}
                            await websocket_manager.manager.send_personal_message(json.dumps(ai_message),current_websocket)
                        asyncio.create_task(deep_handler(websocket,userInput))
